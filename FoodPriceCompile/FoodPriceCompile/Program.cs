            ...
            try
            {
                ...
                for (int i = 0; i < lines.Length; i++)
                {
                    ...
                    if (CatchName && CatchPrice)
                    {
                        ...
                        {
                            newPrice = PriceDice * BottomPrice;
                        }
                        else if (((RealPrice - 10) * PriceMultiIn) > (PriceDice * BottomPrice) && (originalPrice >= (PriceDice * BottomPrice + 0.4))
                            && (!(originalPrice < ((RealPrice - 10) * 0.7)) || ModOpItmes))
                        {
                            newPrice = (RealPrice - 10) * PriceMultiIn + PriceDice * BottomPrice;
                        }
                        else
                        {
